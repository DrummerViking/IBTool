...
$parentModule = 'IBTool'
if (-not $ModuleName) { $ModuleName = 'IBTool.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\IBTool'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\IBTool.psd1"
$prereqHash = @{
