...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'IBTool.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'IBTool.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'IBTool.Client.ProtectedToken'
